...
- `nameLat`: 希腊文/拉丁转写
- `aliases`: 别名数组（含中文别名）
- `era`: 人物生活时代标签：`旧约背景` / `耶稣时期` / `使徒时期` / `时代待审`。`旧约背景`表示此人生活在旧约时期，但因被新约点名而收录。
- `books`: 出现/相关书卷数组（如 `马太福音`）
- `mentions`: 可公开的出现位置数组；每项含 `passage`、`location`、`sourceId`，不含受限制经文全文
...
- `book`: 关系首次涉及书卷标签
- `books`: 关系全部出处涉及的书卷标签；书卷筛选按交集匹配，不只检查首个出处
- `era`: 关系年代/语境
...
- `eraIncludes`: 人物时代包含条件（可为空）；前端按关系任一端人物所属时代匹配
